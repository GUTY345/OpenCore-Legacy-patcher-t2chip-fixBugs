...
class BuildMiscellaneous:
    ...
    def _t2_handling(self) -> None:
        ...
        if self.model not in [
            ...
            # MacBookPro16,2 excluded: Ice Lake — natively supported by macOS Tahoe
            "Macmini8,1",
        ...
        if support.BuildSupport(self.model, self.constants, self.config).get_kext_by_bundle_path("CryptexFixup.kext").get("Enabled") is not True:
            try:
                ...
                sys.exit(3)
        if support.BuildSupport(self.model, self.constants, self.config).get_kext_by_bundle_path("AMFIPass.kext").get("Enabled") is not True:
            try:
        ...
        try:
            ...
            self.config["NVRAM"]["Add"]["7C436110-AB2A-4BBB-A880-FE41995C9F82"]["boot-args"] += " -v rddelay=5 igfxfw=2 igfxonln=1 -disable_ext_panics -no_compat_check"
        except Exception as e:
        ...
        try:
            ...
            already_exists = any(
                p.get("Comment") == "Prevent AppleSEPManager SEP timeout panic"
                for p in self.config["Kernel"]["Patch"]
            )
